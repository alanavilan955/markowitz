...
├── src/
│   ├── __init__.py                 # hace de src un paquete importable
...
├── tests/test_modelo.py            # 15 pruebas, incluida la app
...
python -m pytest -q                # debe terminar con 15 passed
...
**`ModuleNotFoundError` en `from src import graficas as gr`.** La carpeta `src/` necesita su archivo `__init__.py` y la raíz del repositorio debe estar en `sys.path`. La app ya inserta su propia ruta al arrancar y, si aun así falta un módulo, muestra en pantalla cuál es, el contenido de la carpeta y la versión de Python; el mensaje de Streamlit Community Cloud viene censurado. Verifica con:

```bash
git ls-files src
# debe listar: src/__init__.py src/analitica.py src/datos.py src/graficas.py src/markowitz.py
```

Si `src/__init__.py` no aparece, agrégalo y vuelve a subir:

```bash
git add src/__init__.py && git commit -m "Agrega src/__init__.py" && git push
```

**Falta una dependencia.** El nombre que muestre la app debe estar en `requirements.txt`, en la raíz del repositorio y en la rama que desplegaste. Después de editarlo, usa Reboot app en el menú de la app.

**El archivo principal no es `app.py` en la raíz.** En la configuración de la app, el campo de archivo principal debe apuntar a la ruta real, por ejemplo `app.py`.
